...
def delete_a_list_element(list_data, element_value):
    ...
    if element_value not in list_data:
        result = 0
    else:
...
def comparison_list_size(list_data_a, list_data_b):
    ...
    # 한 줄 이상의 코드로 작성 가능하나,
    # 반드시 결과 값을 result 변수에 할당하여 반환
    len_of_list_a = len(list_data_a)
    len_of_list_b = len(list_data_b)

    if len_of_list_a >= len_of_list_b:
        result = list_data_a
    else:
        result = list_data_b
...
def binary_converter(decimal_number):
    ...
    mok = decimal_number
    while mok != 1:
        binary_list.append(mok % 2)
        mok = mok // 2
    binary_list.append(mok)
    binary_str_list = ""
    for i in binary_list:
        binary_str_list += str(i)

    result = binary_str_list[::-1]
    # ==================================
    return result.strip()
